...
htmltools::img(src = knitr::image_uri(system.file('sticker/binneRsticker.png',package = 'binneR')),
  style = 'position:absolute; top:0; right:0; padding:10px; height:180px;')
```
...
dat <- system.file(
	'DataSets/FIE-HRMS/BdistachyonEcotypes/QC01.mzML',
	package = 'metaboData') %>%
	readFiles(scans = 6:12,sranges = list(c(70,1000)),dp = 5)$n
...
Pure <- measures %>%
	filter(bin == 133.01) %>%
	mutate(purity = paste('Purity = ',round(purity,3), sep = ''))
pure <- dat %>%
	filter(bin == Pure$bin)
