...
There are a number of metrics that can be computed that allow the assessment of the quality of a given 0.01 amu bin in terms of the accurate *m/z* peaks present within its boundaries. These include both purity and centrality.
...
Bin purity gives a metric of the spread of accurate *m/z* peaks found within a given bin and can be a signal for the presences of multiple real spectral peaks within a bin.
...
p$impure <- ggplot(impure,aes(x = mz)) +
	...
	theme_bw() +
	xlim(Impure$bin - 0.005,Impure$bin + 0.005) +
	ggtitle(paste('p',Impure$bin,'   ',Impure$centrality,sep = '')) +
	xlab('m/z') +
	ylab('Density')
...
Whereas bin n104.03 as low centrality with a single peak that is located very close to the upper boundary of the bin and could indicate that it has been split between this bin and bin p94.04.
