...
pacman::p_load(binneR,metaboData,mzR,ggplot2,dplyr,magrittr)
```
...
parameters <- detectParameters(files,nCores = 2)
parameters
```
...
analysis <- binneRlyse(files,info,binParameters(scans = detectInfusionScans(files,nCores = 2),cls = 'class',nCores = 2,clusterType = 'PSOCK'))
```
