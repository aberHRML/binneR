...
library(binneR)
infusionScans <- detectInfusionScans(list.files(system.file('mzML',package = 'binneR'),full.names = TRUE),
...
```{r exampleChromatograms, fig.width=5,fig.height=4}
plotChromFromFile(list.files(system.file('mzML',package = 'binneR'),full.names = TRUE),
										sranges = list(c(70,1000)),
									scans = infusionScans
									)
```
...
```{r fileList}
file <- list.files(system.file('mzML',package = 'binneR'),full.names = TRUE)
```
...
```{r analysisChromatogram, fig.width=5,fig.height=4}
plotChromatogram(analysis)
```

And the total ion counts using:

```{r analysisTICs, fig.width=5,fig.height=4}
plotTIC(analysis)
```
