...
```{r FIEfingerprint, echo=FALSE, fig.width=7,fig.height=5,fig.align='center'}
file <- filePaths('FIE-HRMS','BdistachyonEcotypes')[1]
chrom <- openMSfile(file)
chrom <- header(chrom)
chrom <- chrom[seq(1,nrow(chrom),2),]
chrom$acquisitionNum <- 1:nrow(chrom)

spectrum <- readFiles(file,scans = detectInfusionScans(file),sranges = list(c(70,1000)),dp = 2)$n

spectrum <- tidyr::gather(tibble::as_tibble(spectrum),'mz','Intensity')
spectrum <- mutate(spectrum,mz = as.numeric(stringr::str_replace(mz,'[:alpha:]','')))
...
p$spectrum <- ggplot(spectrum,aes(x = mz, y = 0, xend = mz, yend = Intensity)) +
	geom_segment() +
	theme_bw() +
	xlab('m/z') +
	ylab(('Abundance')) +
...
Firstly, an intensity matrix can
Secondly, for routine metabolomic fingerprinting experiments, more formal functionality is provided using S4 structures that allow not only for the production of a spectrally binned intensity, but also the extraction of accurate *m/z* and plotting functionality.
...
```{r infoMatchFiles}
FALSE %in% (info$fileName == files)
```

Spectral binning using the default parameters can then be performed with the following.

```{r exampleProcessing,eval=FALSE}
analysis <- binneRlyse(files,info,binParameters(cls = 'class'))
```

```{r processing,echo=FALSE}
analysis <- binneRlyse(files,info,binParameters(cls = 'class',nCores = 2,clusterType = 'PSOCK'))
```
...
dat <- readFiles(system.file(
	'DataSets/FIE-HRMS/BdistachyonEcotypes/QC01.mzML.gz',
...
p$pure <- ggplot(pure,aes(x = mz)) +
	...
	ggtitle(paste(Pure$bin,'\t',Pure$purity)) +
	xlab('m/z') +
	ylab('Density')
...
p$impure <- ggplot(impure,aes(x = mz)) +
	...
	ggtitle(paste(Impure$bin,'\t',Impure$purity)) +
	xlab('m/z') +
	ylab('Density')
...
Impure <- filter(measures,bin == 128.03)
...
p$pure <- ggplot(pure,aes(x = mz)) +
	...
	theme_bw() +
	xlim(Pure$bin - 0.005,Pure$bin + 0.005) +
	ggtitle(paste(Pure$bin,'\t',Pure$centrality)) +
	xlab('m/z') +
	ylab('Density')
...
p$impure <- ggplot(impure,aes(x = mz)) +
	...
	theme_bw() +
	xlim(Impure$bin - 0.005,Impure$bin + 0.005) +
	ggtitle(paste(Impure$bin,'\t',Impure$centrality)) +
	xlab('m/z') +
	ylab('Density')
