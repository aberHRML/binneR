...
```{r libraryLoad,echo=FALSE,include=FALSE,message=FALSE}
library(binneR)
library(metaboData)
library(mzR)
library(ggplot2)
...
The package supports parallel processing using the [future](https://cran.r-project.org/web/packages/future/index.html) package.

By default processing by `binneR` will be done seqentially.
...
See the future package [documentation](https://cran.r-project.org/web/packages/future/vignettes/future-1-overview.html) for more information on the types of parallel backends that are available.
...
* `detectParameters()` - allows the auto detection of processing parameters. `binParameters()` can be use to manually select these parameters.
...
Alternatively, parameters can be initialised using the `binParameters` function as shown below. Parameter values can be specified using this function.
...
alternativeParameters <- parameters
scans(alternativeParameters) <- 6:14
...
The input of this function is a vector of the paths of the data files to process, a tibble containing the sample info and a `BinParameters` object.
...
dat <- readFiles(files[1],scans = detectInfusionScans(files[1]),dp = 5) %>%
	bind_cols() %>%
	tidyr::gather('mz','Intensity') %>%
	mutate(mode = stringr::str_sub(mz,1,1),
...
Bin purity is calculated by

$$p = 1 - \frac{e}{0.5\cdot w} $$

Where *p* is purity, $e$ is the mean absolute error of the accurate *m/z* present within the bin from the bin mean and *w* is the width of the bin in amu.
...
Pure <- measures %>%
	filter(bin == 133.01,mode == 'n') %>%
	mutate(purity = paste('Purity = ',signif(purity,3), sep = ''))
pure <- dat %>%
	filter(bin == Pure$bin, mode == Pure$mode[1]) %>%
	split(1:nrow(.)) %>%
	purrr::map(~{
		tibble(mz = rep(.x$mz,.x$Intensity))
	}) %>%
	bind_rows()

Impure <- measures %>%
	filter(bin == 146.97,mode == 'n') %>%
	mutate(purity = paste('Purity = ',signif(purity,3), sep = ''))
impure <- dat %>%
	filter(bin == Impure$bin, mode == Impure$mode) %>%
	split(1:nrow(.)) %>%
	purrr::map(~{
		tibble(mz = rep(.x$mz,.x$Intensity))
	}) %>%
...
Bin n146.97, that has a reduced purity, clearly has two peaks present.
...
Pure <- measures %>%
	filter(bin == 88.04,mode == 'n') %>%
	mutate(centrality = paste('Centrality = ',signif(centrality,3), sep = ''))
pure <- dat %>%
	filter(bin == Pure$bin, mode == Pure$mode[1]) %>%
	split(1:nrow(.)) %>%
	purrr::map(~{
