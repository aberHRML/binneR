...
The *binneR* package provides a spectral binning approach for routine processing of high resolution FIE-MS metabolomics fingerprinting experiments, the results of which can then be used for subsequent statistical analyses.

Spectral binning consists of the rounding of high resolution fingerprinting data by a specified amu bin width.
FIE-HRMS data consists of a 'plug flow', across which MS signal intensities can be averaged to provide a metabolome fingerprint. 
Spectral binning is applied on a scan by scan basis where the data is rounded to the specified bin width, the signals are then sum aggregated and then their intensities are averaged across the specified scans.
Below is indicated the 'plug flow' region of an example FIE-MS chromatogram and the resulting spectrum when signal intensities are averaged across these scans.


```{r FIEfingerprint, echo=FALSE, fig.width=5,fig.height=4}
...
For a given set of experimental samples, *binneRlyse* will bin these to both 0.01 and 0.00001 amu.
The 0.00001 data will then be aggregated based on a specified class structure from which the modal accurate *m/z* is extracted for each 0.01 amu bin.
...
The example data used here is from the [*metaboData*](https://github.com/jasenfinch/metaboData) package and consists of a comparison of four *B. distachyon* ecotypes.
...
There are two main functions for processing data using *binneR*:
...
*binneR* requires the provision of sample information (info) for the experimental run to be processed.
...
analysis <- binneRlyse(files,info,binParameters(nCores = 2,clusterType = 'PSOCK'))
```
...
*binneR* provides a number of measures that allow the assessment of the quality of a given 0.01 amu bin in terms of the accurate *m/z* peaks present within its boundaries. These include both purity and centrality.
...
measures <- dplyr::summarise(measures,purity = binneR:::binPurity(mz,Intensity),centrality = binneR:::binCentrality(mz,Intensity),Intensity = mean(Intensity))
```
...
```{r binPurity, echo=FALSE,fig.width=5}
...
Bin 133.01, that has a purity very close to 1, has only one peak present.
Bin 98.96, that has a reduced purity, clearly has two peaks present.
...
```{r binCentrality, echo=FALSE,fig.width=5}
...
Bin 88.04 has a high centrality with single peak that is located very close to the center of the bin.
Whereas bin 128.03 as low centrality with a single peak that is located very close to the upper boundary of the bin and has likely been split between this bin and bin 128.04.